...
if scene_type=="synthetic":
  ...
  def load_blender(data_dir, split):
    ...
    images = images[..., :3] # [PATCH] 强制RGB，修复蓝色变透明bug
    ...
    # print("⚡ Force Scaling Poses by 0.033")
    # poses = poses.at[:, :3, 3].set(poses[:, :3, 3] * 0.033)

    
    # [Auto-Scale] 动态注入的缩放参数
    print("⚡ [Data] Applying Dynamic Scale 0.033...")
    poses = poses.at[:, :3, 3].set(poses[:, :3, 3] * 0.033)
